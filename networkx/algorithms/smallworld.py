...
__all__ = ["random_reference",
           "lattice_reference",
           "_random_CL", #FA 20/01/22
           "sigma",
           "omega"] 
...
def _random_CL(G, niter=100, seed=None):
    """ helper function to parallelize random graph generation and analysis. 
    Returns the average clustering and shortest path length of a random graph
    given a reference graph """
    
    Gr = random_reference(G, niter=niter, seed=seed)
    C = nx.transitivity(Gr)
    L = nx.average_shortest_path_length(Gr)
    return [C, L]
...
@not_implemented_for("directed")
@not_implemented_for("multigraph")
def sigma(G, niter=100, nrand=10, seed=None, n_jobs=None, prefer=None):
    ...
    CL = np.array(Parallel(n_jobs=n_jobs, prefer=prefer)(delayed(_random_CL)(G, niter=niter, seed=seed) for i in range(nrand))) # FA 20/02/22
    Cr, Lr = CL[:,0].mean(), CL[:,1].mean()
