...
def fun(subj, output_dir, nw_N, lbl_N):
    ...
    NW_df, NW_tbl = ct.agg_networks(grad, networks, func=np.median,
                                    by_hemisphere=False, label_tbl=True)
    NW_df.iloc[0, :] = 0
    NW_tbl.iloc[0, :] = 0
...
for n1, n2 in pairs:
    ...
    gcca_lr = np.load(f'{gcca_dir}/{group}.gcca.32k_fs_LR.npy').mean(axis=0)
